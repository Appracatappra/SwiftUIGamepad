...
@Observable open class GameMenu {
    ...
    public var activeItems:[GameMenuItem] {
        var results:[GameMenuItem] = []
    ...
    public func addItem(title:String, enabled:Bool = true, backgroundColor:Color = SwiftUIGamepad.defaultBackgroundColor, action:GameMenuItem.ItemAction? = nil) {
        let id = items.count + 1
        let item = GameMenuItem(id: id, title: title, enabled: enabled, backgroundColor: backgroundColor, action: action)
        items.append(item)
    }
}
